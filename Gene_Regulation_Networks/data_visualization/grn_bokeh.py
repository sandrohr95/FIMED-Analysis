...
def gene_regulatory_network_plot_bokeh(network, net_threshold, config, algorithm_name):
    # We put a threshold to obtain a clear graph with the most representatives genes
    limit = network.index.size * net_threshold
    # Create Graph
    G = networkx.from_pandas_edgelist(network.head(int(limit)), 'TF', 'target', 'importance')
    ...
    # Choose a title!
    title = "Estimating Gene Regulatory Network with" + algorithm_name + "method"
